...
public:
	...
	gms_matcher(const std::vector<cv::Point2d> &vkp1, const cv::Size size1, const vector<cv::Point2d> &vkp2, const cv::Size size2, const list<pair<size_t, size_t>> &vDMatches)
	{
		// Input initialize
...
private:
	...
	void ConvertMatches(const list<pair<size_t, size_t>> &vDMatches, vector<pair<int, int>> &vMatches) {
		vMatches.resize(mNumberMatches);
		std::copy(std::begin(vDMatches), std::end(vDMatches), std::back_inserter(vMatches));
	}
